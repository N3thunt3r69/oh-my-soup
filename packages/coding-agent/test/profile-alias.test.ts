...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through oms", async () => {
		...
		expect(result.command).toBe("oms --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("oms-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command oms --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed oms binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("oms-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("oms-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/oms-profiles.fish") ?? "";
		expect(content).toContain("function oms-work --wraps oms");
		expect(content).toContain("command oms --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/oms-profiles.fish");
		expect(files.get(result.configPath)).toContain("function oms-work --wraps oms");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function oms-work");
		expect(content).toContain("& oms --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& oms --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> oms profile alias: oms-work >>>",
					"alias oms-work='command oms --profile=old'",
					"# <<< oms profile alias: oms-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command oms --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> oms profile alias: oms-work >>>", "oms-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base oms command case-insensitively", async () => {
		for (const aliasName of ["oms", "OMS"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid OMS profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/oms-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/oms-profiles.fish'");
	});
