...
// `@oh-my-soup/pi-*` imports through virtual modules backed by live host module
...
describe("legacy-pi bundled virtual module synthesizer (issue #3423)", () => {
	...
	it("emits one ES named export per enumerable namespace key", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@oh-my-soup/pi-coding-agent", modules);
		expect(src).toContain(
			`const __oms_bundled = globalThis[${JSON.stringify(globalKey)}]["@oh-my-soup/pi-coding-agent"];`,
		);
		expect(src).toContain('export const VERSION = __oms_bundled["VERSION"];');
		expect(src).toContain('export const defineTool = __oms_bundled["defineTool"];');
		expect(src).toContain('export const Type = __oms_bundled["Type"];');
	...
	it("forwards `default` through `export default` so default imports survive", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@oh-my-soup/pi-utils", modules);
		expect(src).toContain("export default __oms_bundled.default;");
		// Default and named exports coexist on the same module.
		expect(src).toContain('export const VERSION = __oms_bundled["VERSION"];');
		expect(src).toContain('export const isCompiledBinary = __oms_bundled["isCompiledBinary"];');
	});
	...
	it("throws when asked to synthesize a key the bundled modules do not cover", () => {
		expect(() => __synthesizeLegacyPiBundledSourceWithModules("@oh-my-soup/pi-not-bundled", modules)).toThrow(
			/no bundled module registered for @oh-my-soup\/pi-not-bundled/,
		);
	});
	...
	it("addresses the same globalThis key the install function would stash to", () => {
		...
		expect(src.startsWith(`const __oms_bundled = globalThis[${JSON.stringify(globalKey)}]["typebox"];`)).toBe(true);
	});
	...
	it("end-to-end: synthesized source resolves named bindings against a runtime globalThis entry", () => {
		...
		try {
			...
			// we can read `__oms_bundled` from the returned closure.
			const body = src
				.split("\n")
				.filter(line => line.startsWith("const __oms_bundled"))
				.join("\n");
			const fn = new Function(`${body}; return __oms_bundled;`);
	...
	it("routes Bun plugin resolution through the bundled namespace so onLoad can serve extension imports", async () => {
		using tempDir = TempDir.createSync("@oms-legacy-pi-bundled-virtual-");
		...
		await Bun.write(
			entryPath,
			[
				'import { legacyAnswer } from "oms-legacy-pi-bundled:@oh-my-soup/pi-utils";',
				"process.stdout.write(legacyAnswer);",
				"",
			].join("\n"),
		);

		expect(resolveBundledVirtualSpecifier("@oh-my-soup/pi-utils")).toEqual({
			namespace: "oms-legacy-pi-bundled",
			path: "@oh-my-soup/pi-utils",
		});
		expect(resolveBundledVirtualSpecifier("oms-legacy-pi-bundled:@oh-my-soup/pi-utils")).toEqual({
			namespace: "oms-legacy-pi-bundled",
			path: "@oh-my-soup/pi-utils",
		});
		...
		const plugin: BunPlugin = {
			name: "oms-legacy-pi-bundled-virtual-regression",
			setup(build) {
				build.onResolve({ filter: /^oms-legacy-pi-bundled:.+$/, namespace: "file" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onResolve({ filter: /.*/, namespace: "oms-legacy-pi-bundled" }, args =>
					resolveBundledVirtualSpecifier(args.path),
				);
				build.onLoad({ filter: /.*/, namespace: "oms-legacy-pi-bundled" }, args => {
		...
		expect(onLoadPaths).toEqual(["@oh-my-soup/pi-utils"]);
		...
		expect(stderr).toBe("");
		expect(stdout).toBe("served:@oh-my-soup/pi-utils");
	});
});
