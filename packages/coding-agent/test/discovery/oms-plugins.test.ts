 ...
 *
 * The native `oms` discovery provider only walks `.oms/` and `~/.oms/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `oms-plugins`
 * provider (`src/discovery/oms-plugins.ts`) is what wires those sub-trees
...
import { hookCapability } from "@oh-my-soup/pi-coding-agent/capability/hook";
import { mcpCapability } from "@oh-my-soup/pi-coding-agent/capability/mcp";
import { promptCapability } from "@oh-my-soup/pi-coding-agent/capability/prompt";
import { ruleCapability } from "@oh-my-soup/pi-coding-agent/capability/rule";
import { skillCapability } from "@oh-my-soup/pi-coding-agent/capability/skill";
import { slashCommandCapability } from "@oh-my-soup/pi-coding-agent/capability/slash-command";
import { toolCapability } from "@oh-my-soup/pi-coding-agent/capability/tool";
import type { LoadContext, Provider } from "@oh-my-soup/pi-coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@oh-my-soup/pi-coding-agent/discovery";
import {
	clearOmsExtensionCliRoots,
	injectOmsExtensionCliRoots,
	listOmsExtensionRoots,
	withOmsExtensionRootScope,
} from "@oh-my-soup/pi-coding-agent/discovery/oms-extension-roots";
import { discoverExtensionPaths } from "@oh-my-soup/pi-coding-agent/extensibility/extensions/loader";
import { getConfigRootDir, removeSyncWithRetries, setAgentDir } from "@oh-my-soup/pi-utils";

const PROVIDER_ID = "oms-plugins";
...
function buildExtensionPackage(packageDir: string, skillName = "my-skill"): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), oms: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearOmsExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oms-plugins-"));
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".oms", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".oms", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("`--extension` CLI injection is wired through the same provider", async () => {
	...
	injectOmsExtensionCliRoots([ext], home, project);
...
test("relative CLI roots rebind when resume switches projects", async () => {
	...
	injectOmsExtensionCliRoots([`./${relativeRoot}`], home, project);

	const destinationContext = { cwd: destination, home, repoRoot: destination };
	const roots = await listOmsExtensionRoots(destinationContext);
...
test("explicit-only CLI roots replace stale state and exclude every ambient package source", async () => {
	...
	const installed = path.join(home, ".oms", "plugins", "node_modules", "installed-extension");
	...
	writeFile(path.join(project, ".oms", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
	writeFile(path.join(home, ".oms", "agent", "settings.json"), JSON.stringify({ extensions: [userExt] }));
	writeFile(
		path.join(home, ".oms", "plugins", "package.json"),
		JSON.stringify({ name: "oms-plugins", dependencies: { "installed-extension": "1.0.0" } }),
	);

	injectOmsExtensionCliRoots([stale], home, project);
	injectOmsExtensionCliRoots([ext], home, project, { mode: "explicit-only", replace: true });

	const roots = await listOmsExtensionRoots(ctx());
	const skills = await loadFromPlugin<{ name: string }>(skillCapability.id, ctx());
...
test("invocation scopes isolate concurrent SDK roots and merge ambient roots only when requested", async () => {
	...
	injectOmsExtensionCliRoots([staleCli], home, project);

	const firstEntered = Promise.withResolvers<void>();
	const secondEntered = Promise.withResolvers<void>();
	const [firstRoots, secondRoots] = await Promise.all([
		withOmsExtensionRootScope([ext], "explicit-only", async () => {
			firstEntered.resolve();
			await secondEntered.promise;
			return listOmsExtensionRoots(ctx());
		}),
		withOmsExtensionRootScope([otherExplicit], "explicit-only", async () => {
	...
	const mergedRoots = await withOmsExtensionRootScope([ext], "merge", () => listOmsExtensionRoots(ctx()));
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".oms", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".oms", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `oms plugin link`/`install`)", async () => {
	...
	writeFile(
		...
		JSON.stringify({ name: "oms-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `oms`/`pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `oms.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".oms", "plugins");
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "oms-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "oms-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `oms plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `oms install
