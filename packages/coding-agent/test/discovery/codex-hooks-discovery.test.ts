 ...
 * in any stranger script (Codex hook scripts, scratch files, …) killed OMS
...
import { type Hook, hookCapability } from "@oh-my-soup/pi-coding-agent/capability/hook";
...
describe("codex hook discovery", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "oms-codex-hooks-home-"));
		tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "oms-codex-hooks-cwd-"));
	...
	test("skips untyped Codex hook scripts so they never reach the extension loader (#3680)", async () => {
		...
		// OMS-shaped pre-*/post-* files but do not match the prefix.
