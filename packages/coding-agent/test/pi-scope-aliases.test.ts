 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@oh-my-soup`
 ...
 *   - `@plannotator/pi-extension`         ⇒ `@oh-my-soup/pi-agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@oh-my-soup/pi-coding-agent/...`
 *
...
const canonicalCodingAgent = Bun.resolveSync("@oh-my-soup/pi-coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@oh-my-soup/pi-coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@oh-my-soup/pi-utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@oh-my-soup/pi-tui", import.meta.dir);
// Subpath: upstream `pi-ai/oauth` re-exported `utils/oauth/index`; our pi-ai now
// exposes the same surface at the real `@oh-my-soup/pi-ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@oh-my-soup/pi-ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@oh-my-soup/pi-ai/oauth`.
	{
