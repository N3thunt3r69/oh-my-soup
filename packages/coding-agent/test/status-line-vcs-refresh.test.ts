...
import type { StatusLineSettings } from "@oh-my-soup/pi-coding-agent/modes/components/status-line";
...
import type { GitHeadState, GitRefHead, GitRepository } from "@oh-my-soup/pi-coding-agent/utils/git";
...
describe("StatusLineComponent applyCwdChange re-points watcher ownership", () => {
	...
	type StatsListener = (current: nodeFs.Stats, previous: nodeFs.Stats) => void;
	...
	const statsOf = (value: number) => ({ mtimeMs: value, ino: value, size: value }) as nodeFs.Stats;
	...
	it("falls back to bounded polling when the new cwd has no repository", () => {
		...
		expect(onBranchChange).toHaveBeenCalledTimes(1);
		expect(component.getTopBorder(80).content).not.toContain("branch-a");
		component.dispose();
	});
});
