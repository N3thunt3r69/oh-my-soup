...
import {
	...
	isInternalUrlPrefix,
} from "@oh-my-soup/pi-coding-agent/modes/internal-url-autocomplete";
import { PromptActionAutocompleteProvider } from "@oh-my-soup/pi-coding-agent/modes/prompt-action-autocomplete";
...
describe("internal-url-autocomplete", () => {
	...
	describe("getInternalUrlSuggestions", () => {
		...
		it("carries the candidate description through", async () => {
			...
			expect(result!.items[0]).toMatchObject({ value: "rule://python", description: "roboms rules" });
		});
