...
describe("createSessionManager — cross-project --resume", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "oms-xproj-"));
...
describe("runRootCommand — cross-project --resume", () => {
	...
	beforeEach(async () => {
		...
		root = await fsp.mkdtemp(path.join(os.tmpdir(), "oms-xproj-root-"));
	...
	it("re-resolves the model scope from the resumed project's enabledModels after the switch", async () => {
		...
		// Launch scope had no patterns, so every resolution is post-switch. An
		// empty first result gets the resolver's normal discovery retry; both
		// passes must use the destination project's settings.
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "oms-moved-xproj-"));
