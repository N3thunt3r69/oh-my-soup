...
import { patch } from "@oh-my-soup/pi-coding-agent/utils/git";
...
describe("joinPatch", () => {
	...
	test("preserves existing part terminators", () => {
		const parts = ["line1\n\n", "line2\n", "line3"];
		const result = patch.join(parts);
...
describe("parseFileDiffs + patch.join binary round-trip", () => {
	...
	test("preserves the binary terminator when the binary block is last", () => {
		const diff = textBlock + binaryBlock;
		const rebuilt = patch.join(parseFileDiffs(diff).map(file => file.content));
		expect(rebuilt).toBe(diff);
		expect(rebuilt.endsWith("zc$@zAB0000\n\n")).toBe(true);
	});

	test("preserves the binary terminator when another file follows", () => {
