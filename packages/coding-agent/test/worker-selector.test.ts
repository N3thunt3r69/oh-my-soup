...
// The worker-host re-entry seam dispatches any `__oms_worker_*` selector to
...
describe("worker selector dispatch", () => {
	...
	it("fails with a nonzero exit and stderr error on an unknown selector", async () => {
		...
		await runCli(["__oms_worker_does_not_exist"]);

		expect(process.exitCode).toBe(1);
		expect(stderr).toHaveBeenCalledWith("Error: unknown worker selector: __oms_worker_does_not_exist\n");
	});
