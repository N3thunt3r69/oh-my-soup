...
describe("global --profile flag", () => {
	...
	beforeEach(() => {
		...
		configDir = `.oms-profile-cli-test-${Snowflake.next()}`;
	...
	afterEach(async () => {
		...
		if (originalOmsProfileEnv === undefined) {
			delete process.env.OMS_PROFILE;
		} else {
			process.env.OMS_PROFILE = originalOmsProfileEnv;
		}
	...
	it("activates a profile inherited from OMS_PROFILE at run time", async () => {
		const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		setProfile(undefined);
		process.env.OMS_PROFILE = "work";
		delete process.env.PI_PROFILE;
	...
	it("installs a shell alias and exits before command dispatch", async () => {
		const installSpy = vi.spyOn(profileAliasCli, "installProfileAlias").mockResolvedValue({
			...
			aliasName: "oms-work",
			profile: "work",
			command: "oms --profile=work",
			reloadedWith: ". '/home/me/.bashrc'",
		});
		const outSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);

		await runCli(["--profile", "work", "--alias", "oms-work", "--version"]);
	...
	it("installs a shell alias when launch is explicit", async () => {
		...
		await runCli(["launch", "--profile", "work", "--alias", "oms-work", "--version"]);
	...
	it("installs a shell alias when acp is explicit", async () => {
		...
		await runCli(["acp", "--profile", "work", "--alias", "oms-work", "--version"]);
	...
	it("loads profile agent .env before command modules import pi-utils env", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "oms-profile-cli-env-"));
		try {
			const home = path.join(root, "home");
			const configDir = ".oms-profile-cli-env";
			...
			await Bun.write(path.join(defaultAgentDir, ".env"), "OMS_PROFILE_BOOTSTRAP_SENTINEL=default\n");
			await Bun.write(path.join(profileAgentDir, ".env"), "OMS_PROFILE_BOOTSTRAP_SENTINEL=work\n");
			...
			await Bun.write(
				...
				[
					...
					'process.stdout.write("\\nSENTINEL=" + (Bun.env.OMS_PROFILE_BOOTSTRAP_SENTINEL ?? ""));',
				].join("\n"),
			);
			...
			delete childEnv.OMS_PROFILE;
			delete childEnv.PI_PROFILE;
			delete childEnv.PI_CODING_AGENT_DIR;
			delete childEnv.OMS_PROFILE_BOOTSTRAP_SENTINEL;
	...
	it("surfaces an invalid OMS_PROFILE env as a clean error, not an import crash", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "oms-profile-cli-env-bad-"));
		try {
			const home = path.join(root, "home");
			...
			expect(stderr).toContain("Invalid OMS profile");
