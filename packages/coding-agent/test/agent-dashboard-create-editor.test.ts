...
import { AgentDashboard } from "@oh-my-soup/pi-coding-agent/modes/components/agent-dashboard";
...
async function makeTempCwd(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-agent-dashboard-"));
	tempDirs.push(dir);
	return dir;
}
