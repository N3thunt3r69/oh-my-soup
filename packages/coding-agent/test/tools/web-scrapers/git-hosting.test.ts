...
describe("parseGitHubUrl — Actions", () => {
	it("classifies a workflow run URL", () => {
		const gh = parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions/runs/27070071296");
		expect(gh).toEqual({ type: "actions-run", owner: "can1357", repo: "oh-my-soup", runId: 27070071296 });
	});

	it("classifies a job URL using the web-form singular `job` segment", () => {
		const gh = parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions/runs/27070071296/job/79897931171");
		expect(gh).toEqual({
			type: "actions-job",
			owner: "can1357",
			repo: "oh-my-soup",
	...
	it("classifies a job URL using the API-form plural `jobs` segment", () => {
		const gh = parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions/runs/27070071296/jobs/79897931171");
	...
	it("does not treat non-run Actions URLs (e.g. workflow files) as runs/jobs", () => {
		expect(parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions/workflows/ci.yml")?.type).toBe("other");
		expect(parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions")?.type).toBe("other");
	});

	it("does not misparse a run URL with a non-numeric id", () => {
		expect(parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/actions/runs/latest")?.type).toBe("other");
	});
...
describe("parseGitHubUrl — commit", () => {
	...
	it("accepts an abbreviated SHA", () => {
		expect(parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/commit/c1a1cb6")).toEqual({
			type: "commit",
			owner: "can1357",
			repo: "oh-my-soup",
			ref: "c1a1cb6",
	...
	it("falls back to `other` for a bare /commit segment with no SHA", () => {
		expect(parseGitHubUrl("https://github.com/pickpocket/oh-my-soup/commit")?.type).toBe("other");
	});
});
