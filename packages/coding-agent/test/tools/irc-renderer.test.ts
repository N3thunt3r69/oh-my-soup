...
import type { IrcMessage } from "@oh-my-soup/pi-coding-agent/irc/bus";
import { getThemeByName } from "@oh-my-soup/pi-coding-agent/modes/theme/theme";
import { type CoordinationDetails, hubToolRenderer } from "@oh-my-soup/pi-coding-agent/tools/hub";
...
describe("hubToolRenderer list", () => {
	...
	it("uses roster counts when the page is empty but parked peers exist", async () => {
		const uiTheme = await theme();
		const rendered = lines(
			hubToolRenderer.renderResult(
				{
