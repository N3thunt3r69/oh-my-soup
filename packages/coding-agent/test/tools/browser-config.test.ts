...
describe("browser backend defaults", () => {
	it("uses browser.cdpUrl before cmux or headless when app is omitted", async () => {
		const script = [
			...
			"let capturedKind;",
			'const registryModule = import.meta.resolve("@oh-my-pi/pi-coding-agent/tools/browser/registry");',
			...
			"  },",
			"}));",
			'const supervisorModule = import.meta.resolve("@oh-my-pi/pi-coding-agent/tools/browser/tab-supervisor");',
			...
			"  runInTab: async () => ({}),",
			"}));",
			'const { BrowserTool } = await import("@oh-my-pi/pi-coding-agent/tools/browser");',
