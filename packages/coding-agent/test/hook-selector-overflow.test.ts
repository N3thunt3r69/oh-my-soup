...
import { HookSelectorComponent } from "@oh-my-soup/pi-coding-agent/modes/components/hook-selector";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to OMS-native only by migrating reusable shared AI instructions into .oms/AGENTS.md, .oms/rules, .oms/skills, and .oms/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .oms while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand OMS resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand OMS resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.oms without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Oh My Soup in terminal",
				description:
