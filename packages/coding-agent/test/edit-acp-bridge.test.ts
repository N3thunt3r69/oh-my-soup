...
} from "@oh-my-soup/pi-coding-agent/edit";
import { HashlineFilesystem } from "@oh-my-soup/pi-coding-agent/edit/hashline/filesystem";
import { resolveLocalUrlToPath } from "@oh-my-soup/pi-coding-agent/internal-urls";
import type { WritethroughCallback } from "@oh-my-soup/pi-coding-agent/lsp";
...
describe("HashlineFilesystem ACP fs routing", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-acp-hashline-"));
	...
	it("keeps a local sandbox artifact addressed by absolute path off the ACP bridge", async () => {
		...
		// the OMS-owned artifact must be written to disk, never pushed to the editor.
		const { bridge, spy: bridgeSpy } = makeBridge();
		const session = createSession(tmpDir, {
			bridge,
...
describe("executeHashlineSingle model-visible payload under write-time drift", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-acp-hashline-e2e-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executeReplace ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-acp-replace-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executePatchSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-acp-patch-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
