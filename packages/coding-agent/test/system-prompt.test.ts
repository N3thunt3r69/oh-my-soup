...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oms-gpu-probe-"));
	try {
		...
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$OMS_GPU_PROBE_COUNT"\nif [ -n "$OMS_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$OMS_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$OMS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$OMS_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$OMS_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$OMS_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$OMS_GPU_PROBE_SLEEP" ]; then exec sleep "$OMS_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.OMS_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.OMS_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			OMS_GPU_PROBE_COUNT: probeCountPath,
			OMS_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		if (options.sleepSeconds === undefined) {
			delete env.OMS_GPU_PROBE_SLEEP;
		} else {
			env.OMS_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.OMS_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.OMS_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.OMS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.OMS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.OMS_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.OMS_GPU_PROBE_VALID_OUTPUT;
		}
