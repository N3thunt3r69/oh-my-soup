 ...
 * `oms list` / `oms remove` / `oms marketplace <sub>` / `oms uninstall …` etc.
 * as top-level commands, but only `oms install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `oms list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `oms plugin list`). #4845 extended the same footgun to the multi-word
 * documented grammar: `oms marketplace add xyz` leaked the whole argv to the
 * model as a prompt.
 *
 ...
 * `oms plugin <action>` command rather than leaking to the model — while
 * genuine prose prompts that merely begin with one of these words still fall
 * through to `launch`.
 *
 * Imported via a relative path (not the `@oh-my-soup/pi-coding-agent` alias) so the
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `oms list` hints at `oms plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("oms plugin list");
	});

	test("bare `oms remove` hints at `oms plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("oms plugin uninstall");
	});
	...
	test("multi-word `oms marketplace add xyz` hints at `oms plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		const result = resolveCliArgv(["marketplace", "add", "xyz"]);
		expect(result).not.toEqual({ argv: ["launch", "marketplace", "add", "xyz"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("oms plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `oms plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "oms plugin marketplace"],
			["discover", "oms plugin discover"],
			["upgrade", "oms plugin upgrade"],
			["uninstall", "oms plugin uninstall"],
			["enable", "oms plugin enable"],
			["disable", "oms plugin disable"],
		] as const) {
