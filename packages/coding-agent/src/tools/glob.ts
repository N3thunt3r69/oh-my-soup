...
export class GlobTool implements AgentTool<typeof findSchema, GlobToolDetails> {
	...
	async execute(
		...
		// Preparation rejects immediately on caller abort. Once every filesystem
		// stat settles, detach this proxy before launching native scans so execute
		// drains each native worker through the real caller signal. Custom
		// operations have no signal API and retain immediate-abort coverage.
			...
			const preparedTargets: NativePreparedTarget[] = await Promise.all(
				targets.map(async target => {
					...
					try {
						...
						// "Path not found" instead of leaking the raw errno.
						if (isEnoent(error) || hasFsCode(error, "ENAMETOOLONG")) {
							if (isSingle) throw new ToolError(`Path not found: ${scopePath}`);
