...
- `agent(..., detach=True)` — spawn WITHOUT waiting: returns at admission with `{"id", "handle": "agent://<id>", "job_id"}`. The child runs as a background job that outlives the eval cell; it is listed by `hub` jobs, its result auto-delivers when it settles, and its full output stays readable at `agent://<id>` (e.g. `output(id)` in a later cell). Use blocking `agent()` for fan-outs you consume in the same cell; use `detach` for long work you check on later.
...
Everything except `detach` runs INLINE and synchronously inside the eval call — no resume, no separate progress app. Each eval call is one well-scoped fan-out; chain several across calls and turns for multi-phase work, reading each result before you decide the next phase.
</helpers>
