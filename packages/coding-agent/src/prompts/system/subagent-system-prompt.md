...
- Discovery: the roster above is a bounded live (running+idle) view plus a parked count, never parked names or task labels. `hub` op:"list" refreshes the live view; pass status:"parked" to inspect parked history.
