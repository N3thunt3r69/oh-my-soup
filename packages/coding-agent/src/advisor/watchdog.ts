...
import { getAgentDir, isEnoent, logger, prompt } from "@oh-my-soup/pi-utils";
 ...
 * `.oms/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.oms/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .oms/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, ".oms", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const ownerDir = baseName === ".oms" ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || baseName === ".oms") {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .oms folder, and user agent dir.
