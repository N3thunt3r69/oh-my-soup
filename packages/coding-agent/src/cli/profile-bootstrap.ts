 ...
 * `@oh-my-soup/pi-utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `oms --system-prompt --profile
 ...
 * `oms --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`oms --print --profile work`).
 */
 ...
 * positionals are never stolen (`oms grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `oms launch --profile work` and
 * `oms acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `oms --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`oms --some-ext-bool --profile work`
		...
		// activates (`oms --print --profile work`).
