...
import type * as MnemopiNs from "@oh-my-soup/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@oh-my-soup/pi-mnemopi";
import type * as MnemopiCoreNs from "@oh-my-soup/pi-mnemopi/core";
import type { LocalModelInitializer } from "@oh-my-soup/pi-mnemopi/core";
...
/**
 * Lazily load `@oh-my-soup/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@oh-my-soup/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@oh-my-soup/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@oh-my-soup/pi-mnemopi/core");
