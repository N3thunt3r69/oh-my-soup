...
export default {
	...
	async isAvailable(session: ToolSession, options?: BackendProbeOptions): Promise<boolean> {
		const availability = await checkPythonKernelAvailability(session.cwd, readInterpreterSetting(session), options);
	...
	async execute(code: string, opts: ExecutorBackendExecOptions): Promise<ExecutorBackendResult> {
		...
		const executorOptions: PythonExecutorOptions = {
			...
			stateSnapshot: readSetting<boolean>(opts.session, "python.stateSnapshot"),
