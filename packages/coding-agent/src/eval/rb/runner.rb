...
# OMS Ruby runner — subprocess wrapper used by the coding-agent host.
#
...
$__oms_out_mutex = Mutex.new
$__oms_raw_stderr = (STDERR.dup rescue STDERR)
$__oms_current_rid = nil
$__oms_capture_rid = nil
$__oms_exec_count = 0
$__oms_active_exec = 0
$__oms_silent = false

begin
  $__oms_frame_io = STDOUT.dup
  $__oms_frame_io.sync = true
  __oms_stdout_cap_r, __oms_stdout_cap_w = IO.pipe
  STDOUT.reopen(__oms_stdout_cap_w)
  STDOUT.sync = true
  __oms_stdout_cap_w.close
  $__oms_stdout_capture_read = __oms_stdout_cap_r
rescue StandardError
  $__oms_frame_io = STDOUT
  ($__oms_frame_io.sync = true) rescue nil
  $__oms_stdout_capture_read = nil
end

begin
  __oms_stderr_cap_r, __oms_stderr_cap_w = IO.pipe
  STDERR.reopen(__oms_stderr_cap_w)
  STDERR.sync = true
  __oms_stderr_cap_w.close
  $__oms_stderr_capture_read = __oms_stderr_cap_r
rescue StandardError
  $__oms_stderr_capture_read = nil
end
...
begin
  $__oms_proto_stdin = STDIN.dup
  STDIN.reopen(File.open(File::NULL, "r"))
rescue StandardError
  $__oms_proto_stdin = STDIN
end
...
def __oms_emit(frame)
  ...
  $__oms_out_mutex.synchronize do
    $__oms_frame_io.write(line)
    $__oms_frame_io.write("\n")
    $__oms_frame_io.flush
  end
rescue StandardError
  nil
end
...
def __oms_emit_stream(kind, text)
  rid = $__oms_current_rid
  if rid.nil?
    ($__oms_raw_stderr.write(text) rescue nil)
    return
  end
  __oms_emit("type" => kind, "id" => rid, "data" => __oms_scrub(text))
end

def __oms_emit_display(bundle, kind = "display")
  rid = $__oms_current_rid
  return if rid.nil?
  __oms_emit("type" => kind, "id" => rid, "bundle" => bundle)
end

def __oms_emit_status(op, data = {})
  status = { "op" => op.to_s }
  data.each { |k, v| status[k.to_s] = v }
  __oms_emit_display({ "application/x-oms-status" => status }, "display")
end

OMS_IMAGE_MIMES = %w[image/png image/jpeg].freeze
...
def __oms_image_payload(content)
  require "base64"
  s = content.to_s
  return s.gsub(/\s+/, "") if __oms_base64?(s)
...
def __oms_sniff_image_mime(bytes)
  b = bytes.to_s.b
...
# bundle is always JSON-safe before it reaches __oms_emit.
def __oms_normalize_bundle(hash)
  bundle = {}
  hash.each do |key, val|
    k = key.to_s
    bundle[k] =
      if OMS_IMAGE_MIMES.include?(k)
        __oms_image_payload(val)
...
def __oms_finalize_bundle(bundle, value)
  bundle["text/plain"] ||= __oms_scrub((value.inspect rescue value.class.name))
  bundle
end

# Rich-display resolution for non-collection objects. Honors the repo
# `to_oms_mime` convention first, then the IRuby protocol
...
def __oms_rich_mime_bundle(value)
  if value.respond_to?(:to_oms_mime)
    mime = (value.to_oms_mime rescue nil)
    return __oms_finalize_bundle(__oms_normalize_bundle(mime), value) if mime.is_a?(Hash) && !mime.empty?
  end
  if value.respond_to?(:to_iruby_mimebundle)
    ...
    return __oms_finalize_bundle(__oms_normalize_bundle(data), value) if data.is_a?(Hash) && !data.empty?
  end
  if value.respond_to?(:to_iruby)
    pair = (value.to_iruby rescue nil)
    if pair.is_a?(Array) && pair.size == 2 && !pair[0].nil?
      return __oms_finalize_bundle(__oms_normalize_bundle({ pair[0].to_s => pair[1] }), value)
    end
  end
  ...
  if value.respond_to?(:to_png)
    ...
    return __oms_finalize_bundle({ "image/png" => __oms_image_payload(png) }, value) if png
  end
  jpeg_method = %i[to_jpeg to_jpg].find { |m| value.respond_to?(m) }
  if jpeg_method
    jpg = (value.public_send(jpeg_method) rescue nil)
    return __oms_finalize_bundle({ "image/jpeg" => __oms_image_payload(jpg) }, value) if jpg
  end
  if value.respond_to?(:to_blob)
    blob = (value.to_blob rescue nil)
    if blob.is_a?(String) && (mime = __oms_sniff_image_mime(blob))
      return __oms_finalize_bundle({ mime => __oms_image_payload(blob) }, value)
    end
  end
...
# Other objects may expose a rich representation via `to_oms_mime` or the IRuby
# protocol (`to_iruby`/`to_iruby_mimebundle`); otherwise they fall back to inspect.
def __oms_mime_bundle(value)
  case value
  when String
    { "text/plain" => __oms_scrub(value) }
  when Hash, Array
    safe = begin
    ...
    if safe.nil?
      { "text/plain" => __oms_scrub(value.inspect) }
    else
      { "application/json" => safe, "text/plain" => __oms_scrub(value.inspect) }
    end
  when nil
    { "text/plain" => "nil" }
  else
    __oms_rich_mime_bundle(value) || { "text/plain" => __oms_scrub(value.inspect) }
  end
end

def __oms_present(value, kind = "display")
  __oms_emit_display(__oms_mime_bundle(value), kind)
end
...
def __oms_start_capture_drain(io, kind)
  return if io.nil?
  Thread.new do
    loop do
      chunk =
      ...
      rid = $__oms_capture_rid
      if rid.nil?
        ($__oms_raw_stderr.write(chunk) rescue nil)
      else
        __oms_emit("type" => kind, "id" => rid, "data" => __oms_scrub(chunk))
      end
    end
  end
end

def __oms_start_parent_watchdog
...
def __oms_install_idle_sigint
  Signal.trap("INT", "IGNORE") rescue nil
end

def __oms_install_exec_sigint
  Signal.trap("INT", "DEFAULT") rescue nil
end

def __oms_begin_exec
  $__oms_active_exec += 1
  __oms_install_exec_sigint
end

def __oms_end_exec
  $__oms_active_exec -= 1 if $__oms_active_exec > 0
  __oms_install_idle_sigint if $__oms_active_exec.zero?
end
...
def __oms_apply_request_runtime(req)
  cwd = req["cwd"]
  ...
  env = req["env"]
  if env.is_a?(Hash)
    OMS_MANAGED_ENV_KEYS.each do |key|
...
def __oms_should_display_result?(src)
  ...
  last = __oms_ast_last(node)
  return true if last.nil?
  !OMS_NON_DISPLAY_NODES.include?(last.type)
end
...
def __oms_emit_error(rid, exc, name_override = nil)
  ...
  __oms_emit(
    ...
    "id" => rid,
    "ename" => ename,
    "evalue" => __oms_scrub(evalue),
    "traceback" => traceback.map { |l| __oms_scrub(l) },
  )
end

def __oms_handle_request(req)
  rid = req["id"].to_s
  $__oms_current_rid = rid
  $__oms_capture_rid = rid
  $__oms_silent = req["silent"] == true
  $__oms_exec_count += 1
  count = $__oms_exec_count
  __oms_emit("type" => "started", "id" => rid)

  status = "ok"
  cancelled = false
  begin
    begin
      __oms_apply_request_runtime(req)
      src = req["code"].to_s
    rescue Exception => e # rubocop:disable Lint/RescueException
      __oms_emit_error(rid, e)
      __oms_emit("type" => "done", "id" => rid, "status" => "error", "executionCount" => count, "cancelled" => false)
      return
    end
    ...
    begin
      ...
      unless $__oms_silent || value.nil? || !__oms_should_display_result?(src)
        __oms_present(value, "result")
      end
    rescue Interrupt => e
      cancelled = true
      status = "error"
      __oms_emit_error(rid, e, "Interrupt")
    rescue SystemExit => e
      status = "error"
      __oms_emit_error(rid, e)
    rescue Exception => e # rubocop:disable Lint/RescueException
    ...
    __oms_emit("type" => "done", "id" => rid, "status" => status, "executionCount" => count, "cancelled" => cancelled)
  ensure
    $__oms_capture_rid = nil if $__oms_capture_rid == rid
    $__oms_current_rid = nil
  end
end
...
def __oms_main
  $stdout = OmsStreamProxy.new("stdout", STDOUT, 1)
  $stderr = OmsStreamProxy.new("stderr", STDERR, 2)
  __oms_install_idle_sigint
  __oms_start_parent_watchdog
  __oms_start_capture_drain($__oms_stdout_capture_read, "stdout")
  __oms_start_capture_drain($__oms_stderr_capture_read, "stderr")

  $__oms_proto_stdin.each_line do |raw|
    line = raw.strip
    next if line.empty?
    req =
      begin
    ...
    __oms_handle_request(req) if req.is_a?(Hash)
  end
end
