 ...
 * value would shadow an entire settings group. Values at schema leaves,
...
export function dropSettingsGroupShadows(data: RawSettings, sourcePath: string, basePrefix = ""): RawSettings {
	...
	for (const key of Object.keys(data)) {
		...
		const settingPath = basePrefix === "" ? key : `${basePrefix}.${key}`;
		if (!Object.hasOwn(SETTINGS_GROUP_ONLY_PREFIXES, settingPath)) {
			result[key] = value;
			continue;
		}
		...
		result[key] = dropSettingsGroupShadows(value as RawSettings, sourcePath, settingPath);
	}
	return result;
}
...
export class Settings {
	...
	static init(options: SettingsOptions = {}): Promise<Settings> {
		...
		const promise = options.readOnly === true ? instance.#loadReadOnly() : instance.#load();
	...
	async #loadProjectSettings(): Promise<RawSettings> {
		...
		const projectConfigPath = path.join(this.#cwd, ".oms", "config.yml");
...
export function resetSettingsForTest(): void {
	...
	globalInstancePromise = null;
	clearBoundSettingsMethods();
	// Persistent Settings instances open the process-wide AgentStorage singleton.
	// Close it before test-owned agent directories are removed; on Windows the
	// live SQLite handle otherwise keeps the directory locked after reset.
