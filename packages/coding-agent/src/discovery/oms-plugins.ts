/**
 * OMS extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by oms's standard
 * discovery surfaces. The native `oms` provider in `builtin.ts` only walks
 * `.oms/` and `~/.oms/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `oms` provider (100) so an
 * extension package never shadows the user's own `.oms/` configuration on
 * dedup.
 *
 * @see ./oms-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@oh-my-soup/pi-utils";
...
import { listOmsExtensionRoots, type OmsExtensionRoot } from "./oms-extension-roots";
import { resolvePluginStdioPaths } from "./substitute-plugin-root";

const PROVIDER_ID = "oms-plugins";
const DISPLAY_NAME = "OMS Extension Packages";
const DESCRIPTION =
 ...
 */
async function allowedRoots(ctx: LoadContext, surface: "skills" | "mcp" | "other"): Promise<OmsExtensionRoot[]> {
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await allowedRoots(ctx, "other");
	const tasks: Array<{ root: OmsExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: OmsExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[oms-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[oms-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[oms-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
