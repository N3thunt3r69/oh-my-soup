 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `oms-plugins.ts`.
 *
 * CLI-provided paths are injected via {@link injectOmsExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listOmsExtensionRoots} to mirror what
...
import { getAgentDir, isEnoent, logger, tryParseJson } from "@oh-my-soup/pi-utils";
...
export interface OmsExtensionRoot {
...
export interface InjectOmsExtensionCliRootOptions {
	 ...
	 * contribute sibling capabilities through the `oms-plugins` provider.
	 */
	mode?: OmsExtensionRootMode;
 ...
 * {@link listOmsExtensionRoots} against that invocation's active cwd.
 */
export function withOmsExtensionRootScope<T>(
	paths: readonly string[],
	mode: OmsExtensionRootMode,
	callback: () => T,
): T {
 ...
 * the registered set; {@link clearOmsExtensionCliRoots} resets for tests.
 */
export function injectOmsExtensionCliRoots(
	paths: readonly string[],
	home: string,
	cwd: string,
	options: InjectOmsExtensionCliRootOptions = {},
): void {
...
export function clearOmsExtensionCliRoots(): void {
	injectedCliRoots = [];
	injectedCliRootMode = "merge";
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedOmsExtensionCliRoots(): readonly OmsExtensionRoot[] {
 ...
 *    via {@link injectOmsExtensionCliRoots}
 * 2. Project `<cwd>/.oms/settings.json#extensions`
 * 3. User `~/.oms/agent/settings.json#extensions`
 * 4. Enabled npm/link plugins installed under `<plugins>/node_modules/` (for
 *    `oms install <pkg>` / `oms plugin install` / `oms plugin link`). Marketplace
 ...
 */
export async function listOmsExtensionRoots(ctx: LoadContext): Promise<OmsExtensionRoot[]> {
 ...
 * "Claude Code Marketplace" and "OMS Extension Packages".
 */
