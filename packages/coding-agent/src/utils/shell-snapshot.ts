...
echo "# Shell snapshot - generated by oms agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__oms_funcs" | __oms_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __oms_sq_quote __oms_emit_export_for __oms_emit_referenced_exports 2>/dev/null
unset __oms_funcs __oms_qbuf __oms_qout __oms_sq __oms_xv __oms_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), uid === undefined ? "oms-shell-snapshots" : `oms-shell-snapshots-${uid}`);
