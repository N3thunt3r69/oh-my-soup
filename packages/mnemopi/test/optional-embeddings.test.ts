...
} from "@oh-my-soup/pi-mnemopi/core/embeddings";
	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`Oh-My-Soup/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://github.com/pickpocket/oh-my-soup");
				expect(request.headers.get("x-openrouter-title")).toBe("Oh-My-Soup");
