...
import { OmsTypeError } from "../src/errors";
...
describe("zod-like parsing", () => {
	it("exposes callable omstype schemas with JSON Schema metadata", () => {
		...
		expect(schema({ website: "https://github.com/pickpocket/oh-my-soup" })).toEqual({
			name: "Ada",
			website: "https://github.com/pickpocket/oh-my-soup",
		});
	...
	it("dispatches min and max by string, number, and array kind", () => {
		...
		expect(() => z.boolean().min(1)).toThrow(OmsTypeError);
	});

	it("supports string and number refinements plus nullable and optional values", () => {
		expect(z.string().regex(/^oms$/).url().safeParse("oms").success).toBe(false);
		expect(z.string().url().parse("https://github.com/pickpocket/oh-my-soup")).toBe(
	...
	it("validates records with string key and value schemas", () => {
		...
		expect(() => z.record(z.number() as never, z.string())).toThrow(OmsTypeError);
