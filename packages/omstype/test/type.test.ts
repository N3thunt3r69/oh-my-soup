...
import { OmsErrors, Type, type } from "../src";
...
function normalized(result: unknown): unknown {
	return result instanceof OmsErrors ? { error: result.summary } : result;
}
...
describe("validation", () => {
	...
	it("rejects wrong primitive, bad literal, broken bound, and missing key with path-aware errors", () => {
		...
		if (missing instanceof OmsErrors) {
			expect(missing[0].path).toEqual(["path"]);
			expect(missing.summary).toContain("path");
		}
		expect(tool({ path: "x", mode: "delete" })).toBeInstanceOf(OmsErrors);
		expect(tool({ path: "x", mode: "read", offset: 0 })).toBeInstanceOf(OmsErrors);
		expect(tool({ path: "x", mode: "read", offset: 1.5 })).toBeInstanceOf(OmsErrors);
		expect(tool({ path: "x", mode: "read", tags: ["a", 1] })).toBeInstanceOf(OmsErrors);
	});
	...
	it("bounds: numbers (exclusive/inclusive) and string length", () => {
		...
		expect(s({ t: 0, name: "ok" })).toBeInstanceOf(OmsErrors);
		expect(s({ t: 3601, name: "ok" })).toBeInstanceOf(OmsErrors);
		expect(s({ t: 1, name: "" })).toBeInstanceOf(OmsErrors);
		expect(s({ t: 1, name: "toolongname" })).toBeInstanceOf(OmsErrors);
	});
	...
	it("array suffix binds outside a trailing bound: string>0[]", () => {
		...
		expect(s({ files: [""] })).toBeInstanceOf(OmsErrors);
		expect(s({ files: "" })).toBeInstanceOf(OmsErrors);
	});
	...
	it("null/undefined union members", () => {
		...
		expect(s("x")).toBe("x");
		expect(s(5)).toBeInstanceOf(OmsErrors);
	});
});
...
describe("methods", () => {
	...
	it(".narrow() with ctx.mustBe records the expectation", () => {
		...
		expect(bad).toBeInstanceOf(OmsErrors);
		if (bad instanceof OmsErrors) expect(bad.summary).toContain("a body unless deleting");
	});

	it(".narrow() runs after morphs (sees defaults)", () => {
		const s = type({ n: "number = 5" }).narrow(v => v.n === 5);
		expect(s({})).toEqual({ n: 5 });
		expect(s({ n: 6 })).toBeInstanceOf(OmsErrors);
	});
	...
	it(".and() merges object schemas", () => {
		...
		expect(s({ b: 2 })).toBeInstanceOf(OmsErrors);
	});
	...
	it("type.enumerated builds literal unions from runtime arrays", () => {
		...
		expect(s("py")).toBe("py");
		expect(s("rs")).toBeInstanceOf(OmsErrors);
	});
...
describe("embedded schemas", () => {
	...
	it("union failures discriminate object members by literal props", () => {
		...
		for (let i = 0; i < JIT; i++) {
			...
			expect(out).toBeInstanceOf(OmsErrors);
...
describe("advanced ArkType compatibility", () => {
	it("re-exports recursive scopes through the Ark compatibility facade", () => {
		...
		expect(invalid).toBeInstanceOf(OmsErrors);
		if (invalid instanceof OmsErrors) expect(invalid[0].path).toEqual(["next", "value"]);
	});
	...
	it("resolves recursive scopes, modules, utility generics, and runtime generics", () => {
		...
		expect(bad).toBeInstanceOf(OmsErrors);
		if (bad instanceof OmsErrors) expect(bad[0].path).toEqual(["manager", "name"]);
		...
		expect(box("number")({ value: "2" })).toBeInstanceOf(OmsErrors);
	});
	...
	it("supports fixed, optional, defaulted, and variadic tuple elements", () => {
		...
		expect(variadic(["x", true])).toBeInstanceOf(OmsErrors);
	...
	it("validates Date literals and bounds before and after JIT compilation", () => {
		...
		for (let index = 0; index < JIT; index++) {
			...
			expect(exact(new Date("2024-01-03T00:00:00.000Z"))).toBeInstanceOf(OmsErrors);
			expect(bounded(new Date("2024-01-01T00:00:00.000Z"))).toEqual(new Date("2024-01-01T00:00:00.000Z"));
			expect(bounded(new Date("2023-12-31T23:59:59.999Z"))).toBeInstanceOf(OmsErrors);
		}
	});
	...
	it("supports disjointness-aware intersections and semantic comparison", () => {
		...
		expect(bounded(0)).toBeInstanceOf(OmsErrors);
	...
	it("maps and selects structural nodes and distributes unions", () => {
		...
		expect(distributed(["c"])).toBeInstanceOf(OmsErrors);
	});
	...
	it("exposes nested keyword modules and configurable error codes", () => {
		...
		expect(type.parse.url("https://github.com/pickpocket/oh-my-soup")).toBeInstanceOf(URL);
...
describe("Standard Schema V1", () => {
	it("validates synchronously with morphs and path-aware issues", () => {
		...
		expect(std.vendor).toBe("omstype");
	...
	it("materializes root defaults for undefined input at the standard boundary", () => {
		...
		expect(staticDefault(5)).toBeInstanceOf(OmsErrors);
...
describe("type.withJsonSchema", () => {
	it("emits the override verbatim even when embedded, and still validates", () => {
		...
		expect(inner("c")).toBeInstanceOf(OmsErrors);
		expect(object({ mode: "b" })).toEqual({ mode: "b" });
		expect(object({ mode: "c" })).toBeInstanceOf(OmsErrors);
	});
