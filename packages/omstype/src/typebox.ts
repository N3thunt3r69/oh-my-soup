import { OmsTypeError } from "./errors";
import type { Def, EmbeddableSchema } from "./ir";
import { type NarrowContext, type Type as OmsType, type ToJsonSchemaOptions, type } from "./type";
...
const OPTIONAL_INNER = Symbol("omstype.typebox.optionalInner");
const OBJECT_INFO = Symbol("omstype.typebox.objectInfo");
 ...
 * omstype schemas included (those carry no legacy compat members).
 *
...
export type TTyped<T> = OmsType<T> & LegacyTypeBoxCompat<T>;
...
interface RuntimeType<T> extends OmsType<T> {
...
function withLegacyCompat<T>(schema: OmsType<T>): CompatRuntime<T> {
...
function checkFiniteOption(name: string, value: number | undefined): void {
	if (value !== undefined && !Number.isFinite(value)) throw new OmsTypeError(`${name} must be finite`);
}
...
function tString(opts?: StringOpts): TString {
	...
	if (opts?.pattern !== undefined) {
		...
		try {
			...
			throw new OmsTypeError(`invalid regular expression pattern ${JSON.stringify(opts.pattern)}`);
		}
...
function tNumber(opts?: NumberOpts, integer = false): TNumber {
	...
	if (opts?.multipleOf !== undefined && opts.multipleOf <= 0)
		throw new OmsTypeError("multipleOf must be greater than zero");
...
function tIntersect<const E extends readonly AnySchema[]>(
	...
	try {
		...
		if (error instanceof OmsTypeError) return applyMeta(validateAll(), opts);
		throw error;
	}
...
function tObject<const P extends Record<string, AnySchema>>(properties: P, opts?: ObjectOpts): TObject<P> {
	...
	for (const key in properties) {
		...
		// omstype (like ArkType) rejects `key?` with a default, and a default
...
function requireObject(schema: AnySchema, operation: string): ObjectInfo {
	...
	if (!info) throw new OmsTypeError(`Type.${operation} requires a schema created by Type.Object`);
	return info;
}
...
function tUnsafe<T = unknown>(_jsonSchema: Record<string, unknown> = {}): TUnsafe<T> {
	...
	// omstype cannot honestly implement that contract without importing a second validator.
	return withLegacyCompat(type.unknown as OmsType<T>);
}
