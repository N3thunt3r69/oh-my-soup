 ...
 * - failure returns an `OmsErrors` with a single fast-fail entry
 */
import { MISSING, OmsErrors } from "./errors";
...
/**
 * Validate `value` against `ir`; returns output value or `OmsErrors`.
...
function fail(path: PropertyKey[], expected: string, data: unknown): OmsErrors {
	const storedPath = path.length === 0 ? undefined : path.length === 1 ? path[0] : [...path];
	return new OmsErrors(storedPath, expected, data);
}
...
function checkNode(ir: IR, v: unknown): boolean {
	switch (ir.k) {
		...
		case "sub":
			return !(ir.schema.run(v) instanceof OmsErrors);
	}
}
...
function visitFinish(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	...
	if (!(out instanceof OmsErrors) || ir.cfg === undefined) return out;
...
function visitNode(ir: IR, v: unknown, path: PropertyKey[]): unknown {
	switch (ir.k) {
		...
		case "refine": {
			...
			if (base instanceof OmsErrors) return base;
			try {
				const result = ir.pred(base);
				if (result instanceof OmsErrors) return path.length === 0 ? result : prefixAll(result, path);
		...
		case "morph": {
			...
			if (input instanceof OmsErrors) return input;
			const context = {
			...
			if (output instanceof OmsErrors) return output;
		...
		case "union": {
			...
			let targeted: OmsErrors | undefined;
			let targetCount = 0;
			for (const member of ir.members) {
		...
		case "array": {
			...
			let errors: OmsErrors | undefined;
			for (let index = 0; index < v.length; index++) {
				path.push(index);
				const element = visit(ir.el, v[index], path);
				path.pop();
				if (element instanceof OmsErrors) {
			...
			for (let index = prefixCount; index < ir.prefix.length; index++) {
				...
				if (item.hasDefault && morph) {
					...
					if (item.defFactory && typeof payload === "function") {
						...
						path.pop();
						if (resolved instanceof OmsErrors) {
...
function prefixAll(errs: OmsErrors, path: PropertyKey[]): OmsErrors {
 ...
 * "A or B" expectation.
 */
export function unionFail(ir: IR & { k: "union" }, v: unknown, path: PropertyKey[], expected?: string): OmsErrors {
	...
	if (ir.members.every(member => unwrapBase(member).k === "object")) {
		const branches = ir.members.flatMap(member => {
			...
			return result instanceof OmsErrors ? [[...result]] : [];
		});
		if (branches.length !== 0) {
			...
			if (common.length !== 0) {
				for (const entry of common) {
					...
					alternatives.push(
						new OmsErrors(entry.path, [...expectations].join(" or "), entry.data, { preserveActual: true }),
					);
				}
			} else {
				for (const branch of branches) {
...
function discriminateFailure(members: IR[], value: unknown, path: PropertyKey[]): OmsErrors | undefined {
	...
	for (const candidate of candidates) {
		...
		if (exact.length === 1) {
			...
			return result instanceof OmsErrors ? result : undefined;
		}
