 ...
 * - failure allocates a single `OmsErrors` (`E(path, expected, data)`); path
...
class CompiledMorphContext implements MorphContext {
	...
	error(expectation: string): OmsErrors {
		return new OmsErrors(this.#path, expectation, this.#data);
	}

	reject(expectation: string): OmsErrors {
...
class Builder {
	...
	buildAllows(ir: IR): (value: unknown) => value is unknown {
		...
		return make(this.#refs, OmsErrors, own);
	}
}

function prefixErrors(errs: OmsErrors, parts: PropertyKey[]): OmsErrors {
	for (let i = parts.length - 1; i >= 0; i--) errs.prefix(parts[i]);
	return errs;
}

const kWalk = Symbol("omstype.boundWalk");
...
export function compileAllows(ir: IR): (value: unknown) => value is unknown {
	...
	if (validator === undefined) {
		...
		allowsCache.set(root, ((value: unknown) =>
			built === undefined ? !(walk(root, value) instanceof OmsErrors) : built(value)) as (
			value: unknown,
		) => value is unknown);
