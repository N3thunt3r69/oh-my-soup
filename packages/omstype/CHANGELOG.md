...
- Reimplemented the Zod compatibility facade (`@oh-my-soup/omstype/zod`) to run purely on internal mechanics, removing the dependency on `zod`.
...
- `.narrow()`/`.filter()` boolean overloads accept `OmsErrors` returns, so `cond || ctx.reject(...)` recipes typecheck.
...
- Introduced omstype, an ArkType-compatible schema validation library featuring a lazy JIT runtime that compiles specialized validators on the third call for ultra-fast hot-path validation and low construction overhead.
...
- Added TypeBox-style (@oh-my-soup/omstype/typebox) and Zod-style (@oh-my-soup/omstype/zod) authoring adapters that produce native omstype schemas.
