...
// SQLite error classifiers live in pi-utils so the credential store and model
// cache share one implementation; re-export them here to preserve the existing
// `@oh-my-soup/pi-ai/auth-storage` API.
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent oms startups can race against WAL recovery and the schema
