...
describe("error-id classification", () => {
	...
	it("classifies Cursor plan gates only for the Cursor provider", () => {
		for (const errorMessage of [
			...
			const cursorId = AIError.classifyMessage(
				message({ provider: "cursor", model: "cursor-grok-4.6", errorMessage }),
			);
			expect(AIError.is(cursorId, AIError.Flag.AccountPolicy)).toBe(true);
			expect(AIError.is(cursorId, AIError.Flag.ContentBlocked)).toBe(true);
			expect(AIError.retriable(cursorId)).toBe(false);

			const relayedId = AIError.classifyMessage(
				message({ provider: "openrouter", model: "cursor-grok-4.6", errorMessage }),
			);
