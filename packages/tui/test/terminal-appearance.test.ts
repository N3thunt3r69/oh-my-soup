...
import { extractPrintableText } from "@oh-my-soup/pi-tui/keys";
...
describe("ProcessTerminal OSC 11 appearance detection", () => {
	...
	it("shutdown balances the single kitty push performed on detection", () => {
		...
		const kittyPushes = new Set(["\x1b[>1u", "\x1b[>3u", "\x1b[>5u", "\x1b[>7u", "\x1b[>31u"]);
		const pushes = writes.filter(w => kittyPushes.has(w)).length;
