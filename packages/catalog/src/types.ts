 ...
 * their estimate/default-tokenizer policy. Values name tokenizer generations:
 * DeepSeek V3 through V4 share `"deepseek-v3"`; Kimi K2 through K3 share
 * `"kimi-k2"`.
 */
...
export interface Model<TApi extends Api = Api> {
	...
	reasoning: boolean;
	/**
	 * Exact local tokenizer family resolved from model identity or supplied
	 * explicitly by a catalog source. Absent leaves fallback policy to callers.
	 */
	tokenizer?: ModelTokenizer;
	input: ("text" | "image")[];
	/**
	 * Decoder family used for image inputs when it has narrower format support
	 * than OMS's general image pipeline. `stb` local backends reject WebP.
	 */
	 ...
	 * output limit OMS cannot discover — sending the wrong value triggers 400s
	 ...
	 * provider client. The `baseUrl` must point at an `oms auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized oms installs (e.g. roboms slots) to route every
