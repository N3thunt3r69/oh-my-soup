...
export function buildOpenAICompat(spec: ModelSpec<"openai-completions">): ResolvedOpenAICompat {
	...
	const compat: ResolvedOpenAICompat = {
		...
		// xAI reasoning models reject penalties and stop on chat completions.
...
export function buildOpenAIResponsesCompat(spec: OpenAIResponsesSpecLike): ResolvedOpenAIResponsesCompat {
	...
	const compat: ResolvedOpenAIResponsesCompat = {
		...
		// `detail: "original"` (400 / 422). Other hosts preserve native-resolution
		// frames. Copilot is host-matched; paid xAI and OAuth share the host family.
		...
		// xAI `/v1/responses` rejects presence/frequency penalties for every model.
		supportsPenaltyAndStopParams: !isXaiHost,
		thinkingFormat,
		reasoningDisableMode: resolveReasoningDisableMode(thinkingFormat),
		omitReasoningEffort: false,
		// Request encrypted xAI reasoning and replay it on later turns.
		includeEncryptedReasoning: true,
		filterReasoningHistory: isOpenRouter && isAnthropicModel,
	...
	if (isXaiHost) {
		...
		// xhigh-capable Grok must keep its native tier unmapped even when stale
		// generated/catalog compat carried the older xhigh/max clamp.
