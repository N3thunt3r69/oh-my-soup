...
describe("LiteLLM provider discovery", () => {
	...
	test("routes only OpenAI-backed rich models through Responses", async () => {
		const fetchMock = vi.fn(async (input: string | URL | Request) => {
			...
			if (url === "http://primary:4000/model_group/info") {
				return Response.json({
					data: [
						{ model_group: "team-reasoner", providers: ["openai"], supports_vision: false },
						{
						...
						{ model_group: "mixed-gpt", providers: ["openai", "azure"], supports_vision: false },
						{ model_group: "claude-proxy", providers: ["anthropic"], supports_vision: false },
					],
				});
			}
		...
		for (const id of ["team-reasoner", "configured-reasoner", "backend-reasoner", "base-reasoner"]) {
			expect(models?.find(model => model.id === id)?.api).toBe("openai-responses");
		}
	...
	test("continues rich discovery when API routing evidence is initially unknown", async () => {
		const calls: string[] = [];
		const fetchMock = vi.fn(async (input: string | URL | Request) => {
			const url = inputUrl(input);
			calls.push(url);
			if (url === MODELS_DEV_URL) return Response.json({});
			if (url === "http://primary:4000/model_group/info") {
				return Response.json({ data: [{ model_group: "opaque-alias", supports_vision: false }] });
			}
			if (url === "http://primary:4000/v2/model/info") {
				return Response.json({
					data: [
	...
	test("preserves conservative mixed-provider evidence within one rich endpoint", async () => {
		const fetchMock = vi.fn(async (input: string | URL | Request) => {
			const url = inputUrl(input);
			if (url === MODELS_DEV_URL) return Response.json({});
			if (url === "http://primary:4000/model_group/info") {
