...
Every key below is defined in the settings schema; `oms config list` shows the full set with current values. Defaults and enum values are taken from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
| `compaction.keepRecentTokens` | number  | `-1` _(auto)_ | Recent tokens always preserved. `-1` scales with the model window: `max(20000, 10%)` capped at `65536`. Explicit positive values are used as-is.                                                                                          |
...
| ----------------------------------- | ------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `providers.webSearchOrder`          | array   | `[]`      | Provider IDs in priority order for `web_search` (`perplexity`, `gemini`, `anthropic`, `codex`, `zai`, `exa`, `jina`, `kagi`, `tavily`, `brave`, `kimi`, `parallel`, `synthetic`, `searxng`, `google`, `yandex`, …). Duplicates and unknown IDs are ignored; unlisted providers retain their built-in relative order afterward. Empty = built-in order. Replaces the removed `providers.webSearch` enum (a legacy value migrates to the head of this list). |
| `providers.webSearchTimeoutSeconds` | number  | `60`      | Hard timeout in seconds for each `web_search` provider attempt. Failed or empty searches are attempted up to three times before the automatic chain advances. Use a larger value for slower model-backed providers; values above `300` are capped at five minutes. This is not a whole-chain or whole-provider deadline, and provider-specific upstream or aggregate limits may still be shorter. |
