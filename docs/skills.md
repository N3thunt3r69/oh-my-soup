...
- `description` is required for:
  - native `.oms` provider skill discovery (`requireDescription: true`)
  - `oms-plugins` extension-package skills and the `github` provider (`.github/skills/`), which also pass `requireDescription: true`
...
3. **Managed (auto-learn) skills** (`oms-managed` provider) resolved dead-last, so any same-named enabled authored skill from a provider or custom directory takes precedence
...
1. `native` (priority 100) — `.oms` user/project skills via `src/discovery/builtin.ts`
2. `oms-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:`, `--extension`/`-e`, or installed plugins under `~/.oms/plugins/node_modules`
...
7. `oms-managed` (priority 5) — auto-learn skills under `~/.oms/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
...
The `agents` provider (`.agent[s]/skills`) is the canonical OMS-native location and has its own `enableAgentsUser`/`enableAgentsProject` toggles — disabling Claude/Codex/Pi does **not** turn it off. Providers without a dedicated toggle (`claude-plugins`, `opencode`, `github`, …) are enabled if **any** named third-party source toggle is enabled.
