...
- First resize pass encodes PNG, JPEG, and WebP, then keeps the smallest encoded buffer. Fallback passes encode JPEG and WebP only, again keeping the smaller output. WebP is excluded from both ladders when `OMS_NO_WEBP=1`/`true` (or `excludeWebP` is passed).
- Renderer caps:
