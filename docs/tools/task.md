...
- Agent discovery precedence is first-wins by exact name: project `.oms` agents before user `.oms`, then OMS extension-package `agents/` roots in `listOmsExtensionRoots` order (CLI, project setting, user setting, installed npm/link plugins), Claude marketplace plugin agents (project before user), and bundled agents. Direct `.claude/agents`, `.codex/agents`, and `.gemini/agents` roots are skipped. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
