...
> Open, reuse, close, and script browser tabs against project-shared Chromium, CDP-attached apps, the user's Chrome through the OMS Browser Relay, or cmux surfaces.
...
- **Browser kind**
  - **Headless**: attaches to one project-shared Chromium supervised by the daemon broker (`oms.browser.headless` / `oms.browser.headed` in `hub ps`), applies stealth patches, and creates a fresh page per tab. The daemon stops with the last oms client in the project. Non-CLI hosts launch a private local Chromium instead.
...
- Network
  ...
  - Headless first-use Chromium download uses the in-house `@oh-my-soup/pi-utils/browsers` installer.
