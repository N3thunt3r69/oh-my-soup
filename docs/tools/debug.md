...
IDA setup follows [cellebrite-labs/ida-bridge](https://github.com/cellebrite-labs/ida-bridge): install `ida-bridge` into the selected Python environment, then configure `disasm.ida.installDir` and `disasm.ida.python`. `disasm({ action: "open", file: "/path/to/binary" })` starts the local bridge automatically, launches a dedicated idalib worker, waits for auto-analysis, and returns a target ID ready for `query` or `execute`; separate calls can keep multiple files open simultaneously. No manual server startup or binary loading is required. Raw binaries use a temporary IDB deleted on `close` unless `output_idb` names a persistent `.i64`/`.idb`; `save` and `close` operate on one target without affecting the others. The adapter exposes only clients whose bridge metadata reports `runtime: "idalib"`. `disasm.ida.url` overrides the default `ws://127.0.0.1:8765`; `endpoint`, `ida_dir`, and `python` provide call-level overrides. Prefer `query` for the bridge's shared SQL interface and reserve backend-native `execute` (IDAPython for IDA) for operations SQL cannot express.
