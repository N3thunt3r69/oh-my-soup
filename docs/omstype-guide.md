# omstype Guide (schema authoring in this repo)

Internal schemas use **`@oh-my-soup/omstype`** — an ArkType-compatible validator
with a lazy JIT runtime (`packages/omstype`). Author types with
`import { type } from "@oh-my-soup/omstype"`.


## Why omstype (perf contract)
...
- **omstype** = a callable function with `.toJsonSchema` and `.assert` methods (`isArkSchema`).
...
- Failure returns an `OmsErrors` (array of `OmsError`); `type.errors === OmsErrors`.
...
- Definition mistakes (bad DSL, illegal composition) throw `OmsTypeError` at
  `type()` time.
...
TypeBox-style and Zod-style authoring are backed by the omstype runtime:

```ts
import { Type, type Static } from "@oh-my-soup/omstype/typebox";
import { z } from "@oh-my-soup/omstype/zod";

const User = z.object({ name: z.string() });
type User = z.infer<typeof User>;
```

These produce real omstype schemas with JIT validation and `toJsonSchema`.
