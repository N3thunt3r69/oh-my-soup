...
description: Use when creating a new oms hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
export default function myHook(oms: HookAPI): void {
  oms.on("tool_call", async (event, ctx) => {
...
```ts
oms.on("tool_result", async (event, ctx) => {
...
```ts
oms.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(oms: HookAPI): void {
  oms.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(oms: HookAPI): void {
  oms.on("tool_result", async (event) => {
...
export default function contextFilter(oms: HookAPI): void {
  oms.on("context", async (event) => {
