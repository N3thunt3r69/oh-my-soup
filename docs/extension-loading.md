...
The project root is the native provider's `.oms` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `oms --profile <name>` it becomes `~/.oms/profiles/<name>/agent/extensions` (and it honors `PI_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).
...
- User: the active agent directory's `config.yml` (default `~/.oms/agent/config.yml`; with `--profile <name>`, `~/.oms/profiles/<name>/agent/config.yml`; `PI_CODING_AGENT_DIR` can override the agent directory)
...
1. `package.json` in that directory with `oms.extensions` (or legacy `pi.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
