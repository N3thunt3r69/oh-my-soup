...
//!   `$XDG_STATE_HOME/oms/logs/` on Linux / macOS when the user has migrated to
...
/// Default directory name for OMS's per-user state (overridable via
/// `PI_CONFIG_DIR`, matching `packages/utils/src/dirs.ts`).
const DEFAULT_CONFIG_DIR: &str = ".oms";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/oms/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "oms";
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/oms/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/oms` exists on disk, and
...
/// reads, no fs reads. `oms_dir_exists` decides whether the candidate
/// `<xdg_state_home>/oms` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
	xdg_state_home: Option<&OsStr>,
	agent_dir_override: Option<&OsStr>,
	default_agent_dir: &Path,
	oms_dir_exists: impl FnOnce(&Path) -> bool,
) -> Option<PathBuf> {
	...
	let oms_dir = Path::new(xdg).join(APP_NAME);
	if !oms_dir_exists(&oms_dir) {
		return None;
	}
	Some(oms_dir.join("logs"))
}
#[cfg(any(target_os = "linux", target_os = "macos"))]
...
mod tests {
	...
	fn resolve_logs_dir_honors_relative_pi_config_dir() {
		let dir = resolve_logs_dir(
			...
			Some(OsStr::new(".oms-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.oms-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/oms/logs")));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/oms/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/oms/logs"));
	}
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn xdg_state_logs_skipped_when_oms_dir_missing() {
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			None,
			Path::new("/tmp/pi-natives-test-home/.oms/agent"),
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_oms_by_default() {
	...
	fn default_agent_dir_respects_pi_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/pi-natives-test-home"), Some(OsStr::new(".oms-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.oms-dev/agent"));
	}
