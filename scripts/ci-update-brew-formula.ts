#!/usr/bin/env bun
//
// Render the Homebrew formula for `oms` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (oms-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/oms.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.OMS_REPO ?? "pickpocket/oh-my-soup";
const HOMEPAGE = "https://github.com/pickpocket/oh-my-soup";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["oms-*"].first` then
	// returns `nil` and `bin.install nil => "oms"` raises.
	//
	...
	// not touch the real `/Users/<user>/.oms` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Oms < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/oms-darwin-arm64",
          using: :nounzip
      sha256 "${sums["oms-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/oms-darwin-x64",
          using: :nounzip
      sha256 "${sums["oms-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/oms-linux-arm64",
          using: :nounzip
      sha256 "${sums["oms-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/oms-linux-x64",
          using: :nounzip
      sha256 "${sums["oms-linux-x64"]}"
    end
  end
...
async function main(): Promise<void> {
	...
	const targets = ["oms-darwin-arm64", "oms-darwin-x64", "oms-linux-arm64", "oms-linux-x64"];
