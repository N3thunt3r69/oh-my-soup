...
`scripts/probe.ts` routes through `@oh-my-soup/pi-ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .oms/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .oms/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@oh-my-soup/pi-ai"; // or toolWireSchema(tool)
