...
Typed Python bindings for the `oms --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `oms --mode rpc` flags such as thinking level,
...
from oms_rpc import RpcClient, host_tool
...
```python
from oms_rpc import RpcClient, host_uri
...
```python
from oms_rpc import assistant_text, assistant_text_with_thinking
