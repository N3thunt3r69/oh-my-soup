...
Self-hosted GitHub triage bot. Drives [`oms --mode rpc`](https://github.com/pickpocket/oh-my-soup)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBOMS_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same oms session
...
- **roboms** — FastAPI + sqlite event queue + `WorkerPool` running `oms` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from roboms, executes REST + `git push`.
...
→ `worker.run_task` spawns `oms --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBOMS_MODEL` (CSV).

The agent uses oms's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.oms/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host oms doesn't route through the gateway). robooms lives inside the oh-my-soup
monorepo at `python/roboms/`; both the docker build context and the
`/work/pi` bind mount default to the parent monorepo (`../..`). Override
`PI_ROOT` only if you want a different oh-my-soup checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROBOMS_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROBOMS_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run pi:image                  # build oh-my-soup/pi:dev (one-time / on pi change)
bun run roboms:build && bun run roboms:up
...
comment out `ROBOMS_GH_PROXY_URL` / `ROBOMS_GH_PROXY_HMAC_KEY` and set
`GITHUB_TOKEN`. The two modes are mutually exclusive (`config.py`
rejects a `.env` setting both).

Build invalidation is bounded: editing robooms Python touches only the
runtime layer; editing pi source rebuilds `oh-my-soup/pi:dev`, which
robooms's `Dockerfile.roboms` extends via `FROM ${PI_BASE}`.

### Public URL

robooms does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m roboms serve`. Other commands run
inside the running container:

```bash
docker compose exec roboms roboms triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec roboms roboms replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec roboms roboms status                   # dump issues table
docker compose exec roboms roboms cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run roboms:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`roboms:dev`, `roboms:build`, `roboms:up`, `roboms:down`,
`roboms:logs`, `roboms:restart`, `roboms:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBOMS_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `oms --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `oms` on
...
- gh-proxy has no host port. The `roboms_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBOMS_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBOMS_GIT_AUTHOR_NAME` +
  `ROBOMS_GIT_AUTHOR_EMAIL`. Commit messages carrying shell-literal
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBOMS_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBOMS_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/roboms.log.jsonl`.
...
| Container exits with `PI_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/roboms/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid oh-my-soup checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROBOMS_BOT_LOGIN` does not identify the PAT account's mention handle (production: `robooms`, no `@`/`[bot]`). |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROBOMS_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run pi:image` then `bun run roboms:build`. |
| `No API key found for <provider>` | `~/.oms/agent/models.container.yml` mount missing or provider id mismatch with `ROBOMS_MODEL`. |
...
src/
  ...
  worker.py          synchronous oms RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one ROBOMS_INTEGRATION=1 smoke test
