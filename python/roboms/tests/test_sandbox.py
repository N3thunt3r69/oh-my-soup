...
def test_rename_workspace_branch_refreshes_shared_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "roboms.sandbox._share_git_metadata_with_slots",
...
def test_rename_workspace_branch_runs_git_as_slot_when_permissions_active(
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.subprocess.run", fake_run)
    monkeypatch.setattr("roboms.sandbox._share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)
...
def test_delete_bad_refs_removes_worktree_holding_the_ref(tmp_path: Path) -> None:
    ...
    from roboms.git_ops import _delete_bad_refs
...
def test_ensure_workspace_creates_worktree(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    ws = mgr.ensure_workspace(
        ...
        default_branch="main",
        author_name="roboms-bot",
        author_email="roboms-bot@example.invalid",
    )
...
def test_chown_workspace_noops_when_not_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 1000)
    monkeypatch.setattr(
        "roboms.sandbox.subprocess.run",
...
def test_chown_workspace_noops_off_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "roboms.sandbox.subprocess.run",
        lambda cmd, *, check: calls.append((cmd, check)),
...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared oms group.
    assert calls == [
...
def test_reap_slot_noops_when_permissions_inactive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_reap_slot_kills_slot_uid_on_linux_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox._slot_pids", lambda _uid: (111, 222))
    monkeypatch.setattr("roboms.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))

    tmpdir = _prepare_slot_tmpdir(_workspace(tmp_path), 2001)

    assert tmpdir == tmp_path / ".oms-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".oms-xdg" / "data", tmp_path / ".oms-xdg" / "state", tmp_path / ".oms-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "oms").is_dir()
    assert (tmp_path / ".oms-xdg" / "cache" / "bun-install").is_dir()
...
def test_chown_workspace_normalizes_to_root_when_slots_disabled(
    ...
    monkeypatch.setattr("roboms.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.os.getegid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.subprocess.run", fake_run)
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    monkeypatch.setattr("roboms.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboms.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
    monkeypatch.setattr("roboms.sandbox.subprocess.run", lambda cmd, **_kwargs: calls.append(cmd))

    ws = _workspace(tmp_path)
    bun_cache = ws.root / ".oms-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".oms-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".oms-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".oms-xdg" / "data", ws.root / ".oms-xdg" / "state", ws.root / ".oms-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "oms").is_dir()
...
def test_ensure_workspace_refreshes_permissions_for_retry_slot_and_session(
    ...
    monkeypatch.setattr("roboms.sandbox._chown_workspace", record_chown)
    monkeypatch.setattr("roboms.sandbox._share_git_metadata_with_slots", record_share)
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".oms-tmp",
            ws_root / ".oms-xdg" / "data",
            ws_root / ".oms-xdg" / "data" / "oms",
            ws_root / ".oms-xdg" / "state",
            ws_root / ".oms-xdg" / "state" / "oms",
            ws_root / ".oms-xdg" / "cache",
            ws_root / ".oms-xdg" / "cache" / "oms",
            ws_root / ".oms-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".oms-tmp",
        ws.root / ".oms-xdg" / "data",
        ws.root / ".oms-xdg" / "data" / "oms",
        ws.root / ".oms-xdg" / "state",
        ws.root / ".oms-xdg" / "state" / "oms",
        ws.root / ".oms-xdg" / "cache",
        ws.root / ".oms-xdg" / "cache" / "oms",
        ws.root / ".oms-xdg" / "cache" / "bun-install",
    }
...
def test_remove_workspace_prunes_pool_after_failed_worktree_remove(
    ...
    monkeypatch.setattr("roboms.sandbox._safe_run", fake_safe_run)
...
def test_redact_credentials_strips_userinfo() -> None:
    from roboms.sandbox import redact_credentials
...
def test_git_command_error_redacts_url_in_args_and_stderr(tmp_path: Path) -> None:
    ...
    from roboms.sandbox import _run
...
def test_push_force_with_lease_succeeds_after_local_amend(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    from roboms.git_ops import push as git_push
...
def test_push_force_with_lease_refuses_when_origin_moved(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    force-pushing."""
    from roboms.git_ops import GitCommandError
    from roboms.git_ops import push as git_push
...
def test_run_git_injects_safe_directory_and_subprocess_identity(
    ...
    from roboms.git_ops import _run_git
    ...
    monkeypatch.setattr("roboms.git_ops.subprocess.run", fake_run)
...
def test_run_git_scopes_token_and_scrubs_parent_auth_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from roboms.git_ops import AUTH_ENV_VAR, _run_git
...
def test_run_git_kills_hung_child(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    timeout."""
    from roboms.git_ops import GitCommandError, _run_git
...
# ---------------------------------------------------------------------------
# Partial-clone blob backfill (oh-my-soup#1818)
# ---------------------------------------------------------------------------
...
def test_fetch_ref_backfills_missing_blobs_into_partial_clone(tmp_path: Path) -> None:
    """Regression for oh-my-soup#1818: ``fetch_ref`` is called immediately before
...
def test_ensure_workspace_populates_from_natives_cache(tmp_path: Path, upstream_repo: Path) -> None:
    from roboms.natives_cache import NativesCache, compute_key, target_triple
...
def test_ensure_workspace_cache_miss_is_silent_noop(tmp_path: Path, upstream_repo: Path) -> None:
    from roboms.natives_cache import NativesCache
...
def test_ensure_workspace_acquires_repo_lock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox._run", lambda *a, **k: ok)
    monkeypatch.setattr("roboms.sandbox._safe_run", lambda *a, **k: ok)
    monkeypatch.setattr("roboms.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_safe_run_timeout_returns_124(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox.subprocess.run", boom)
...
def test_ensure_workspace_raises_when_local_branch_probe_times_out(
    ...
    monkeypatch.setattr("roboms.sandbox._safe_run", fake_safe_run)
    monkeypatch.setattr("roboms.sandbox._run", lambda *a, **k: subprocess.CompletedProcess(["x"], 0, "", ""))
    monkeypatch.setattr("roboms.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_ensure_workspace_raises_when_symbolic_ref_probe_times_out(
    ...
    monkeypatch.setattr("roboms.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._provision_runtime_dirs", lambda *a, **k: None)
    monkeypatch.setattr("roboms.sandbox._git_env_for_repo", lambda *a, **k: {})
...
def test_worktree_add_cleans_partial_state_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboms.sandbox._run", boom_run)
    monkeypatch.setattr("roboms.sandbox._safe_run", fake_safe_run)
...
def _seed_reclaimable_workspace(mgr: SandboxManager, repo: str, number: int) -> Path:
    ...
    (ws_root / ".oms-session/session.jsonl").write_text("{}", encoding="utf-8")
    (ws_root / ".oms-xdg/cache/bun-install/pkg.tgz").write_text("x", encoding="utf-8")
    (ws_root / ".oms-xdg/state/oms/state.json").write_text("{}", encoding="utf-8")
    (ws_root / ".oms-tmp/scratch").write_text("x", encoding="utf-8")
...
def test_reclaim_workspace_caches_strips_dep_caches_and_preserves_state(tmp_path: Path) -> None:
    ...
    assert not (ws_root / ".oms-xdg/cache").exists()
    assert not (ws_root / ".oms-tmp").exists()
    assert (ws_root / "repo/src/keep.ts").read_text(encoding="utf-8") == "keep"
    assert (ws_root / ".oms-session/session.jsonl").exists()
    assert (ws_root / ".oms-xdg/state/oms/state.json").exists()
