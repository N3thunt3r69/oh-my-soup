...
from roboms.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboms-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @roboms-bot please look", "roboms-bot") == "hey please look"
    assert extract_mention("@roboms-bot do X", "roboms-bot") == "do X"


@pytest.mark.parametrize("configured_login", ["@robooms", "robooms[bot]", "@robooms[bot]"])
def test_extract_mention_accepts_prefixed_or_app_bot_login(configured_login: str) -> None:
    assert extract_mention("@robooms go ahead", configured_login) == "go ahead"


def test_extract_mention_strips_literal_app_suffix_from_body() -> None:
    assert extract_mention("@robooms[bot] go ahead", "robooms[bot]") == "go ahead"


def test_extract_mention_rejects_extended_literal_app_suffix() -> None:
    assert extract_mention("@robooms[bot]-helper go ahead", "robooms[bot]") is None


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "roboms-bot") is None
    assert extract_mention(None, "roboms-bot") is None
    assert extract_mention("", "roboms-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROBOMS-BOT", "roboms-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @roboms-bot-helper must NOT match @roboms-bot.
    assert extract_mention("@roboms-bot-helper hi", "roboms-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@roboms-bot one, then @roboms-bot two", "roboms-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboms-bot go ahead and push",
            },
            "issue": {"number": 9},
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboms-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboms-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboms-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboms-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
