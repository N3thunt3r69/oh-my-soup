...
import httpx
import pytest
from oms_rpc import HostToolContext, RpcCommandError

from roboms import host_tools
from roboms.db import Database
from roboms.github_client import GitHubClient, IssueIndexEntry, IssueInfo, RepoInfo
from roboms.host_tools import AbortController, ToolBindings, build
from roboms.sandbox import LocalGitTransport, Workspace
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("ROBOMS_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["ROBOMS_GH_PROXY_HMAC_KEY"] == ""
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".oms-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".oms-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".oms-tmp")
    ...
    assert (bindings.workspace.root / ".oms-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.oms-xdg/cache/bun-install")
...
def test_guarded_push_branch_rev_parse_runs_via_repo_command_and_passes_slot_uid(
    ...
    from roboms.git_ops import PushResult
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\troboms-bot@example.invalid\troboms-bot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 999, "user": {"login": "roboms-bot"}, "body": "hi", "created_at": "t"})
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "roboms-bot"}, "body": "hi", "created_at": "t"})
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 321, "user": {"login": "roboms-bot"}, "body": "x", "created_at": "t"})
...
def test_submit_pr_review_posts_summary_only_when_no_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"id": 45, "user": {"login": "roboms-bot"}, "body": "ok", "state": "COMMENTED", "submitted_at": "t"},
        )
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    from roboms.sandbox import SandboxManager
    ...
    try:
        ...
        assert "roboms-bot <roboms-bot@example.invalid>" in msg
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    seed.mkdir()
    env = os.environ | {
        "GIT_AUTHOR_NAME": "roboms-bot",
        "GIT_AUTHOR_EMAIL": "roboms-bot@example.invalid",
        "GIT_COMMITTER_NAME": "roboms-bot",
        "GIT_COMMITTER_EMAIL": "roboms-bot@example.invalid",
    }
    subprocess.run(["git", "init", "--initial-branch=main", str(seed)], check=True, capture_output=True)
    (seed / "README.md").write_text("init\n")
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=roboms-bot@example.invalid",
            "-c",
            "user.name=roboms-bot",
            "commit",
            "-m",
            "init",
        ],
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0] == "roboms-bot <roboms-bot@example.invalid> feat: follow-up", lines
    assert lines[1] == "roboms-bot <roboms-bot@example.invalid> init", lines
...
def test_gh_open_pr_runs_fix_then_check_and_amends_formatter_diff(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "roboms-bot|roboms-bot@example.invalid|feat: initial change"
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "roboms-bot"}, "body": "x", "created_at": "t"},
        )
