...
from roboms import host_tools, persona, pragmas
from roboms.cancellation import register_cancel_hook, unregister_cancel_hook
from roboms.config import Settings
from roboms.db import Database, issue_key
from roboms.git_ops import DirtyState, inspect_dirty_state
...
from roboms.host_tools import AbortController, ToolBindings, _git_identity_env
from roboms.natives_cache import NativesCache
from roboms.natives_cache import compute_key as natives_compute_key
from roboms.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    # `bash` tool could otherwise `printenv` them out of robooms's env.
    "GITHUB_TOKEN",
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".oms/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".oms":
            # ~/.oms/run is slot-writable daemon presence state, not template
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.oms/run`` writable by every sandbox slot.

    oms registers daemon project presence under ``~/.oms/run`` at startup,
    ...
    The tree stays group ``oms``, setgid, group-writable; slot subprocesses
    spawn with umask 0002 so their entries inherit group write.
    """
    if os.geteuid() != 0:
        return
    run_dir = _AGENT_HOME / ".oms" / "run"
    try:
        gid = grp.getgrnam("oms").gr_gid
    except KeyError:
        return
    try:
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the oms subprocess.

    `oms_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an oms JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    def _on_tool_end(event: ToolExecutionEndEvent) -> None:
        ...
        # `tool_name` is transport-normalized by oms_rpc: an xd:// device
    ...
    # can resolve workspace packages (@oh-my-soup/pi-*) and actually run tests.
    ...
    with RpcClient(
        executable=settings.oms_command,
        ...
        extra_groups=["oms"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the oms subprocess
        ...
        # it's idempotent.
        #
        # NOTE: oms_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("oms task exceeded hard timeout")
            if turn is not None and turn.assistant_message is not None:
                ...
                if stop_reason == "error":
                    ...
                    raise RuntimeError(f"oms agent error (stopReason=error): {error_msg}")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
