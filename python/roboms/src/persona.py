...
from roboms.sandbox import Workspace
...
def _load(name: str) -> str:
    return resources.files("roboms.prompts").joinpath(name).read_text(encoding="utf-8")
...
def resume_triage(*, repo: RepoInfo, issue: IssueInfo, workspace: Workspace) -> str:
    """Resume prompt for a `triage_issue` task whose oms session already exists."""
...
def dirty_state_reminder(
    *,
    ...
    leaves changes behind that robooms would otherwise discard. The summary
