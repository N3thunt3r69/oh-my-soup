...
from roboms import github_events, issue_index
from roboms.autoclose import AutocloseScheduler
from roboms.config import Settings, get_settings
from roboms.dashboard import render_index, static_dir, tail_jsonl
from roboms.db import (
...
from roboms.github_client import GitHubError, IssueSummary
from roboms.issue_index import IssueIndexSync
from roboms.manual_triage import (
    InvalidIssueRef,
...
from roboms.proxy_client import GitHubProxyClient, ProxyGitTransport
from roboms.queue import WorkerPool
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "roboms orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "roboms orchestrator requires ROBOMS_GH_PROXY_URL and "
            "ROBOMS_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="roboms", version="0.1.0", lifespan=lifespan)
    ...
    async def replay(
        request: Request,
        x_roboms_token: str | None = Header(None, alias="X-Roboms-Replay-Token"),
        delivery_id: str = "",
    ) -> JSONResponse:
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        ...
        if x_roboms_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set ROBOMS_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        """Browse issues across `ROBOMS_REPO_ALLOWLIST` for the trigger picker.
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_roboms_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in ROBOMS_REPO_ALLOWLIST")
            try:
    ...
    async def api_cancel(
        ...
        """Stop a running event. The oms subprocess is killed; the row lands in
    ...
    async def api_logs(request: Request, limit: int = 400) -> dict[str, Any]:
        ...
        entries = tail_jsonl(cfg.log_dir / "roboms.log.jsonl", limit=capped)
