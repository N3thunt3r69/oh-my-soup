...
PAT injection via `--config-env` (see `roboms.git_ops`); the `ProxyGitTransport`
in `roboms.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.oms-session/`, `context/`, `artifacts/`, `.oms-tmp/`, `.oms-xdg`):
   single-owner. Owned by the active slot UID/GID (`oms-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:oms` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:oms` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from roboms.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `roboms.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.oms-tmp`` (symlink-protection: a user
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``oms``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".oms-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "oms").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.oms-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.oms-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".oms-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `oms-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `oms` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".oms-session"
            ...
            # keeps its `root:oms` ownership (the slot reads through group `oms`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        ...
        `.node` files are LEFT at `root:oms` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `oms`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:oms cache and the slot reads them via
        group `oms`. Everything else (the directory itself, copied
