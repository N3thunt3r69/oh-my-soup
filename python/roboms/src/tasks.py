...
from roboms.db import Database, IssueRow, IssueState, issue_key
...
from roboms.sandbox import GitTransport, SandboxManager
from roboms.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_roboms_directive")
...
async def triage_issue(
    *,
    ...
    if existing is None:
        ...
        # resources (workspace, oms session) to this issue.
        try:
...
async def handle_pr_conversation(
    *,
    ...
    # _roboms_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without oms.
    if directive is None and payload.get("_roboms_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
